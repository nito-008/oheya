...
export const ProfileCarousel = component$<ProfileCarouselProps>(
  ({ initialSlide = 1, profile, track }) => {
    ...
    useVisibleTask$(({ cleanup, track: trackSignal }) => {
      trackSignal(() => initialSlide);
    ...
    return (
      <div class={styles.profileCarousel}>
        <div
          ...
          aria-label={`${profile.name}のプロフィール`}
        >
          <section
            class={`${styles.carouselSlide} ${styles.profileSlide}`}
            aria-label="プロフィール"
          >
